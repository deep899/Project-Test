  ...
  <div style="height: 100vh;">
    <div class="mainContainer">
      ...
      <div class="container secondDiv" style="flex-direction: column;">
        <h3>{{ businessNameHeading }}</h3>
        <div id="inputName">
          <label for="name">{{ businessNameLabel }}</label>
          <input v-model="businessName" type="text" name="name" id="name" :placeholder="businessNamePlaceholder">
        </div>
...
export default {
  data() {
    return {
      ...
      businessNameHeading: 'First Let\'s Give Your Business Name',
      businessNameLabel: 'Enter your Business name',
      businessNamePlaceholder: 'Site Title',
  ...
  methods: {
    continueToCategories() {
      ...
      // axios.post('http://localhost:3300/api/storeBusinessInfo', { businessName: this.businessName })
      //   .then(response => {
      //     console.log('Business name stored successfully');
      //     this.$router.push({ name: 'categories' });
      //   })
      //   .catch(error => {
      //     console.error('Error storing business name:', error);
      //   });
    ...
    // fetchBusinessName() {
    //   axios.get('http://localhost:3300/api/getBusinessInfo')
    //     .then((response) => {
